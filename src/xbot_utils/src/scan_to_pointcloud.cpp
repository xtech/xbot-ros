...
ScanToPointcloud::ScanToPointcloud(const rclcpp::NodeOptions &options)
  ...
  max_cloud_size_ =
      static_cast<size_t>(this->get_parameter("max_cloud_size").as_int());
  odom_buffer_duration_ = rclcpp::Duration::from_seconds(
      this->get_parameter("odom_buffer_duration").as_double());
  ...
  scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
      "scan", 10,
      std::bind(&ScanToPointcloud::scanCallback, this, std::placeholders::_1));
...
void ScanToPointcloud::odomCallback(
...
ScanToPointcloud::getClosestOdom(const rclcpp::Time &stamp) {
  std::lock_guard<std::mutex> lock(odom_mutex_);
...
void ScanToPointcloud::deskewPointCloud(
  ...
  for (int i = 0; i < 10 && !transform_found; ++i) {
    try {
      transform_stamped = tf_buffer_->lookupTransform(
          target_frame_, scan->header.frame_id, scan->header.stamp);
      transform_found = true;
    } catch (tf2::TransformException &ex) {
      // RCLCPP_WARN(this->get_logger(), "Could not transform laser scan: %s",
      // ex.what());
...
void ScanToPointcloud::scanCallback(
    const sensor_msgs::msg::LaserScan::SharedPtr scan) {
    ...
    *cloud_ += *cloud_out;
    ...
    if (cloud_->size() > max_cloud_size_) {
      ...
      cloud_->erase(cloud_->begin(),
                    cloud_->begin() + (cloud_->size() - max_cloud_size_));
    }
